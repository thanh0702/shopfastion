    ...
    <div class="row mb-4">
        <div class="col-sm-2">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="has_sizes" name="has_sizes" {{ $category->sizes ? 'checked' : '' }}>
        ...
        <div class="col-sm-10">
            <textarea class="form-control" id="sizes" name="sizes" rows="3" placeholder="Enter sizes (one per line or comma-separated)" {{ $category->sizes ? '' : 'disabled' }}>{{ old('sizes', $category->sizes ? implode("\n", $category->sizes) : '') }}</textarea>
        </div>
    </div>
